...
[Authorize]
public class OutboundOrdersController : BaseController
{
    private readonly OutboundOrderService _svc;

    public OutboundOrdersController(OutboundOrderService svc) => _svc = svc;
    ...
    public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CreateOrderRequest body)
    {
        ...
        var orderId = await _svc.CreateOutboundOrderAsync(warehouseId, GetRequiredUserId(), GetRequiredUserName(), body.orderNo, body.items);
        return Ok(ApiResponse<object>.Ok(new { orderId }, "出庫單建立成功"));
    }

    [HttpPost("{id:long}/confirm")]
    [Authorize(Roles = "WarehouseAdmin")]
    public async Task<ActionResult<ApiResponse<object>>> Confirm([FromRoute] long id)
        => await _svc.ConfirmOutboundOrderAsync(id, GetRequiredUserId(), GetRequiredUserName())
            ? Ok(ApiResponse<object>.Ok(new { }, "出庫單已確認"))
            : BadRequest(ApiResponse<object>.Fail("僅能確認 Pending 狀態的出庫單"));

    [HttpPost("{id:long}/reject")]
    [Authorize(Roles = "WarehouseAdmin")]
    public async Task<ActionResult<ApiResponse<object>>> Reject([FromRoute] long id, [FromBody] ReasonBody body)
        => await _svc.RejectOutboundOrderAsync(id, GetRequiredUserId(), GetRequiredUserName(), body.reason)
            ? Ok(ApiResponse<object>.Ok(new { }, "出庫單已拒絕"))
            : BadRequest(ApiResponse<object>.Fail("僅能拒絕 Pending 狀態的出庫單"));
        ...
        [FromQuery] int pageSize = 20)
    {
        ...
        var data = await _svc.ListOutboundOrdersAsync(warehouseId, status, orderNo, page, pageSize);
    ...
    public async Task<ActionResult<ApiResponse<OrderDetail>>> Get([FromRoute] long id)
    {
        ...
        var data = await _svc.GetOutboundOrderAsync(id, scope);
        return data != null
            ? Ok(ApiResponse<OrderDetail>.Ok(data))
            : NotFound(ApiResponse<OrderDetail>.Fail("出庫單不存在"));
    }
}
