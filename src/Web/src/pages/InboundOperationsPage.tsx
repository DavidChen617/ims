...
import type { CreateInboundItem, JwtProfile } from '../api/types'
...
export function InboundOperationsPage() {
  ...
  return (
    <div className="space-y-6">
      {canCreate ? <CreateInboundForm token={token} /> : null}
      <PendingInboundList canReview={canReview} token={token} />
    </div>
  )
}

function CreateInboundForm({ token }: { token: string }) {
  ...
  const createInbound = useMutation({
    mutationFn: () => api.createInbound(token, { orderNo, items }),
    onSuccess: async () => {
      setOrderNo('')
      setItems([])
      await queryClient.invalidateQueries({ queryKey: ['inbound-pending'] })
    },
  })
  ...
  return (
    <section className="border border-slate-200 bg-white">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-950">新增進貨申請</h2>
        <p className="text-xs text-slate-500">加入商品品項後送出，等待倉庫管理者審核。</p>
      </div>
      <div className="grid gap-4 p-4">
        ...
        <div className="grid gap-3 border border-dashed border-slate-300 p-3 sm:grid-cols-[1fr_120px_140px_auto]">
          <Field label="商品">
            <select
              className="w-full border border-slate-300 px-3 py-2 text-sm outline-none focus:border-sky-500"
          ...
          <Field label="單價（選填）">
            <input
              ...
              placeholder={productMap.get(productId) ? String(productMap.get(productId)?.price) : ''}
              step="0.01"
              type="number"
              value={unitPrice}
            />
        ...
        <Table columns={['商品編號', '數量', '單價', '操作']}>
          {items.length === 0 ? (
            <EmptyRow colSpan={4} message="尚未加入品項" />
          ) : (
            items.map((item, index) => (
              <tr className="hover:bg-slate-50" key={`${item.productId}-${index}`}>
                ...
                <td className="whitespace-nowrap px-4 py-3 font-mono text-sm">
                  {item.unitPrice != null ? number(item.unitPrice) : `${number(productMap.get(item.productId)?.price)}（預設）`}
                </td>
        ...
        <div>
          <button
            ...
            disabled={createInbound.isPending || !orderNo || items.length === 0}
            onClick={() => createInbound.mutate()}
            type="button"
          >
            <Plus className="h-4 w-4" />
            送出申請
        ...
        <MutationError error={createInbound.error} />
      </div>
    </section>
  )
}
...
function PendingInboundList({ token, canReview }: { token: string; canReview: boolean }) {
  const queryClient = useQueryClient()
  ...
  const pending = useQuery({
    queryKey: ['inbound-pending', page],
    queryFn: () => api.listPendingInboundOrders(token, { page, size: PENDING_PAGE_SIZE }),
  })

  const detail = useQuery({
    enabled: !!selectedId,
    queryKey: ['inbound-order', selectedId],
    queryFn: () => api.getInboundOrder(token, selectedId!),
  })
  ...
  async function refresh() {
    ...
    setRejectReason('')
    await queryClient.invalidateQueries({ queryKey: ['inbound-pending'] })
    await queryClient.invalidateQueries({ queryKey: ['inbound-history'] })
  }

  const confirm = useMutation({
    mutationFn: (id: string) => api.confirmInbound(token, id),
    onSuccess: refresh,
  })

  const reject = useMutation({
    mutationFn: (id: string) => api.rejectInbound(token, id, { reason: rejectReason }),
    onSuccess: refresh,
  })
  ...
  return (
    <section className="border border-slate-200 bg-white">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-950">待審核進貨單</h2>
      </div>
          ...
          <DataState error={detail.error} isLoading={detail.isLoading}>
            {detail.data ? (
              <div className="space-y-4">
                ...
                <Table columns={['商品編號', '商品名稱', '單位', '數量', '單價', '總額']}>
                  {detail.data.items.map((item) => (
                    <tr key={item.productId}>
                      ...
                      <td className="whitespace-nowrap px-4 py-3 font-mono text-sm">{number(item.quantity)}</td>
                      <td className="whitespace-nowrap px-4 py-3 font-mono text-sm">{number(item.unitPrice)}</td>
                      <td className="whitespace-nowrap px-4 py-3 font-mono text-sm">{number(item.totalAmount)}</td>
                    </tr>
                  ))}
                </Table>
